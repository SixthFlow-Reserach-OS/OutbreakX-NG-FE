...
export interface MapProject {
  id: string;
  name: string;
  geojson: GeoJSON.FeatureCollection | string; // Support both formats for backward compatibility
  ownerId: string;
...
export class ProjectService {
  ...
  public readonly activeProjectGeoJSON = computed(() => {
    const project = this.activeProject();
    if (!project) return null;
    return restoreGeoJSONFromFirestore(project.geojson);
  });

  // Observable versions for compatibility
  public readonly projects$: Observable<MapProject[]> =
  ...
  public readonly activeProject$: Observable<MapProject | null> = toObservable(
    this.activeProject
  );

  public readonly activeProjectGeoJSON$: Observable<GeoJSON.FeatureCollection | null> =
    toObservable(this.activeProjectGeoJSON);
  ...
  async createProject(name: string): Promise<void> {
    ...
    try {
      ...
      const projectData = {
        name: name,
        geojson: prepareGeoJSONForFirestore({
   ...
   */
  async updateActiveProjectGeoJSON(
    ...
    try {
      ...
      // Convert GeoJSON to string format for Firestore storage
      const geojsonString = prepareGeoJSONForFirestore(geojson);
  ...
  /**
   * Get active project's GeoJSON as a FeatureCollection object
   */
  getActiveProjectGeoJSON(): GeoJSON.FeatureCollection | null {
    return this.activeProjectGeoJSON();
  }
